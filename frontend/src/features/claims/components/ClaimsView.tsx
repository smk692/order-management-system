...
import MemoModal from '@/shared/components/MemoModal'

const MOCK_CLAIMS = [
  {
    id: 'CLM-001',
    orderId: 'ORD-20250118-005',
    type: 'RETURN',
    channel: 'MALL',
    reason: '단순변심',
    customer: '김민지',
    date: '2025-01-18 16:20',
    status: 'REQUESTED',
    priority: 'HIGH',
  },
  {
    id: 'CLM-002',
    orderId: 'ORD-20250117-042',
    type: 'EXCHANGE',
    channel: 'NAVER',
    reason: '사이즈 오배송',
    customer: '이태민',
    date: '2025-01-18 14:10',
    status: 'PROCESSING',
    priority: 'NORMAL',
  },
  {
    id: 'CLM-003',
    orderId: 'ORD-20250116-012',
    type: 'CANCEL',
    channel: 'COUPANG',
    reason: '배송 지연',
    customer: '최유리',
    date: '2025-01-18 09:30',
    status: 'COMPLETED',
    priority: 'LOW',
  },
]
    ...
    <div className="space-y-8 pb-32 relative">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-2">
            ...
            <p className="text-slate-500 font-medium text-sm">
              취소, 반품, 교환 등 모든 사후 처리 프로세스를 관리합니다.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          <div
            key={i}
            className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm group hover:border-indigo-400 transition-all"
          >
            <div className="flex items-center justify-between mb-4">
              <div
                className={`w-12 h-12 rounded-2xl flex items-center justify-center ${stat.color} transition-transform group-hover:scale-110`}
              >
                {stat.icon}
              </div>
              <span
                className={`text-[10px] font-black px-2 py-1 rounded-lg bg-slate-50 text-slate-500`}
              >
                {stat.change}
              </span>
            </div>
          ...
          <table className="w-full text-left min-w-[1000px]">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/50">
                <th className="py-6 px-8 text-[11px] font-black text-slate-400 uppercase tracking-widest">
                  Claim Info
                </th>
                <th className="py-6 px-4 text-[11px] font-black text-slate-400 uppercase tracking-widest text-center">
                  Type
                </th>
                ...
                >
                  <td className="py-6 px-8">
                    <div className="flex flex-col">
                      <span className="text-sm font-black text-slate-900 group-hover:text-indigo-600 transition-colors italic">
                        {claim.id}
                      </span>
                      <span className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-tight">
                        {claim.orderId} • {claim.customer}
                      </span>
                    </div>
                  </td>
                  ...
                  <td className="py-6 px-6 text-center">
                    <span className="text-[10px] font-black text-slate-400 border border-slate-200 px-3 py-1.5 rounded-xl uppercase tracking-widest">
                      {claim.channel}
                    </span>
                  </td>
                  ...
                  <td className="py-6 px-6 text-center">
                    <span
                      className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border ${
