...
const INITIAL_RULES = [
  {
    id: 'RULE-001',
    name: 'VIP 주문 자동 배정',
    desc: '100만원 이상 주문 건 VIP 전담팀 자동 할당',
    status: 'active',
    trigger: '신규 주문 발생',
    action: '담당자 변경',
    lastRun: '12분 전',
  ...
  },
  {
    id: 'RULE-003',
    name: '장기 미처리 주문 경고',
    desc: '상품준비중 단계에서 48시간 정체 시 관리자 메일 발송',
    status: 'paused',
    trigger: '시간 경과',
    action: '이메일 발송',
    lastRun: '어제',
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm font-medium mt-1">
            반복적인 업무를 트리거 기반의 자동화 규칙으로 최적화합니다.
          </p>
        </div>
        <button
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-slate-50 border-b border-slate-100">
              <tr>
                <th className="py-5 px-8 text-[11px] font-black text-slate-400 uppercase tracking-widest">
                  Rule Name & Desc
                </th>
            ...
            <tbody className="divide-y divide-slate-50">
              {rules.map(rule => (
                <tr
                  key={rule.id}
                  className={`hover:bg-slate-50/50 transition-all ${rule.status === 'paused' ? 'opacity-60' : ''}`}
                >
                  <td className="py-6 px-8">
                    <div className="flex items-center gap-4">
                      <div
                        className={`w-10 h-10 rounded-xl flex items-center justify-center ${rule.status === 'active' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200' : 'bg-slate-200 text-slate-400'}`}
                      >
                        <Zap size={18} />
                      </div>
                      <div>
                  ...
                  <td className="py-6 px-6">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-black px-2 py-1 bg-blue-50 rounded-lg text-blue-600">
                        {rule.action}
                      </span>
                    </div>
                  </td>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
              ...
              <div className="flex items-center justify-between mb-8">
                {[1, 2, 3].map(step => (
                  <div key={step} className="flex flex-col items-center gap-2 relative flex-1">
                    <div
                      className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold z-10 transition-all ${wizardStep >= step ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-300'}`}
                    >
                      {wizardStep > step ? <Check size={18} /> : step}
                    </div>
                    {step < 3 && (
                      <div
                        className={`absolute top-5 left-1/2 w-full h-0.5 -z-0 ${wizardStep > step ? 'bg-indigo-600' : 'bg-slate-100'}`}
                      ></div>
                    )}
                  </div>
                ))}
                ...
                <div className="space-y-4 animate-in fade-in">
                  ...
                  <div className="p-4 bg-blue-50 rounded-2xl border border-blue-100 flex gap-3">
                    ...
                    <p className="text-xs text-blue-700 leading-relaxed font-medium">
                      관리하기 쉬운 이름을 입력하면 나중에 검색이 편리합니다.
                    </p>
                  </div>
                </div>
              )}
            ...
            <div className="p-6 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
              <button
                onClick={() =>
                  wizardStep > 1 ? setWizardStep(wizardStep - 1) : setShowNewRuleModal(false)
                }
              ...
              <button
                onClick={() =>
                  wizardStep < 3 ? setWizardStep(wizardStep + 1) : handleCreateRule()
                }
