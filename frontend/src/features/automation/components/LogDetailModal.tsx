  ...
  const mockJson = {
    ...
    response: {
      ...
      body: {
        ...
        items: [{ sku: 'SKU-00124', qty: 1, price: 89000, currency: 'KRW' }],
        status: 'PAID',
      },
    },
  }
      ...
      <div className="relative bg-[#0F172A] w-full max-w-5xl rounded-[3rem] shadow-2xl border border-slate-700/50 overflow-hidden flex flex-col max-h-[92vh] animate-in zoom-in-95 duration-200">
        ...
        <div className="p-8 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
            ...
            <div>
              <div className="flex items-center gap-3">
                <h2
                  ...
                  className="text-xl font-black text-white italic tracking-tight uppercase"
                >
                  API Intelligence Analyzer
                </h2>
                <span className="px-2 py-0.5 bg-emerald-500/10 text-emerald-500 rounded-lg text-[10px] font-black uppercase tracking-widest border border-emerald-500/20">
                  Success
                </span>
              </div>
              <p className="text-xs text-slate-500 font-mono mt-1 tracking-tight">
                {logId} | TraceID: 4f92-a1b2-c3d4-e5f6
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={copyAsCurl}
              className="px-4 py-2 bg-white/5 hover:bg-white/10 text-slate-400 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all"
            >
              Copy as CURL
            </button>
            <button
              onClick={onClose}
              className="p-3 hover:bg-white/10 rounded-2xl text-slate-500 transition-colors"
            >
              <X size={28} />
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-10 grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => copyToClipboard(mockJson.request)}
                className="p-2 hover:bg-white/5 rounded-xl text-slate-600 hover:text-slate-300 transition-all"
              >
                <Copy size={16} />
              </button>
            </div>
            <div className="bg-slate-950/80 rounded-[2rem] border border-white/5 p-8 font-mono text-[11px] leading-relaxed text-indigo-300/90 overflow-x-auto shadow-inner relative group">
              <Braces
                className="absolute right-6 top-6 opacity-5 group-hover:opacity-10 transition-opacity"
                size={80}
              />
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => copyToClipboard(mockJson.response)}
                className="p-2 hover:bg-white/5 rounded-xl text-slate-600 hover:text-slate-300 transition-all"
              >
                <Copy size={16} />
              </button>
            ...
            <div className="bg-slate-950/80 rounded-[2rem] border border-white/5 p-8 font-mono text-[11px] leading-relaxed text-emerald-300/90 overflow-x-auto shadow-inner relative group">
              <div className="mb-6 flex items-center gap-3 relative z-10">
                <span className="px-3 py-1 bg-emerald-50 text-slate-900 rounded-lg font-black text-[10px] uppercase">
                  200 OK
                </span>
        ...
        <div className="p-8 bg-black/20 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-6">
          ...
          <div className="flex gap-3 w-full sm:w-auto">
            ...
            <button
              onClick={onClose}
              className="flex-1 sm:flex-none px-12 py-4 bg-indigo-600 text-white text-xs font-black uppercase tracking-[0.2em] rounded-2xl hover:bg-indigo-700 transition-all shadow-2xl shadow-indigo-900/50 active:scale-95"
            >
              CLOSE ANALYZER
            </button>
          </div>
