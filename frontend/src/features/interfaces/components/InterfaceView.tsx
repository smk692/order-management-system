...
import LogDetailModal from '@/features/automation/components/LogDetailModal'
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between gap-10">
          <div className="space-y-6">
            <div className="flex items-center gap-3">
              <div className="px-4 py-1.5 bg-emerald-500 rounded-full text-[10px] font-black uppercase tracking-[0.2em] animate-pulse">
                Gateway Live
              </div>
            ...
            <div>
              ...
              <p className="text-slate-400 text-sm mt-2 max-w-md font-medium leading-relaxed">
                실시간 데이터 패킷 수신 및 API 통신 상태를 감시합니다. 장애 발생 시{' '}
                <span className="text-indigo-400 font-black">Debugger</span> 탭에서 페이로드를
                분석하세요.
              </p>
            </div>
          </div>
        ...
        <div className="animate-in fade-in duration-500 space-y-8">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 bg-white p-4 rounded-[2.5rem] border border-slate-200 shadow-sm">
            <div className="flex-1 min-w-[300px] relative">
              ...
              <input
                type="text"
                placeholder="Trace ID, 주문번호, 이벤트명 검색..."
                className="w-full pl-14 pr-4 py-3.5 bg-slate-50 border-none rounded-2xl text-sm font-bold outline-none"
              />
            </div>
            <div className="flex gap-2">
              ...
              {['ALL', 'SUCCESS', 'FAILED'].map(s => (
                <button
                  key={s}
                  className="px-5 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest bg-slate-50 text-slate-400 hover:bg-indigo-50 hover:text-indigo-600 transition-all"
                >
                  {s}
                </button>
              ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-8 bg-white rounded-[3rem] border border-slate-200 shadow-sm overflow-hidden flex flex-col min-h-[600px]">
              <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/30">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <button className="p-2 bg-white rounded-xl border border-slate-100 text-slate-400 hover:text-indigo-600 transition-all shadow-sm">
                    <Trash2 size={16} />
                  </button>
                </div>
              </div>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-slate-50/50 border-b border-slate-100">
                      <th className="py-5 px-8 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                        Time / Status
                      </th>
                      ...
                      <th className="py-5 px-8 text-right text-[10px] font-black text-slate-400 uppercase tracking-widest">
                        Action
                      </th>
                    </tr>
                  </thead>
                      ...
                      >
                        <td className="py-6 px-8">
                          <div className="flex items-center gap-4">
                            ...
                            <span
                              className={`px-2 py-1 rounded-lg text-[10px] font-black border ${log.success ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-rose-50 text-rose-600 border-rose-100'}`}
                            >
                              HTTP {log.status}
                            </span>
                          </div>
                        ...
                        <td className="py-6 px-6">
                          <span className="text-xs font-black text-slate-800 tracking-tight italic uppercase">
                            {log.event}
                          </span>
                        </td>
                        <td className="py-6 px-6">
              ...
              <div className="mt-auto p-6 bg-slate-50/50 border-t border-slate-100 flex justify-center">
                <button className="text-xs font-black text-slate-400 hover:text-indigo-600 uppercase tracking-widest">
                  Show More Debug Logs
                </button>
              </div>
            </div>
            ...
            <div className="lg:col-span-4 space-y-6">
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <div className="flex items-end gap-3">
                        ...
                        <span className="text-emerald-400 text-xs font-bold pb-1 flex items-center gap-1">
              ...
              <div className="bg-white p-8 rounded-[3rem] border border-slate-200 shadow-sm space-y-6">
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <div key={i} className="flex justify-between items-center">
                      ...
                      <span
                        className={`px-2 py-1 rounded-lg text-[10px] font-black uppercase ${s.color}`}
                      >
                        {s.status}
                      </span>
                    </div>
                ...
                <button className="w-full py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] shadow-xl hover:bg-slate-800 active:scale-95 transition-all mt-4">
                  Audit Endpoints
                </button>
              </div>
            </div>
      ...
      {activeView === 'connections' && (
        <div className="animate-in fade-in duration-500 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {globalChannels.map(item => (
            <div
              key={item.id}
              className="bg-white p-8 rounded-[3rem] border border-slate-200 shadow-sm group hover:border-indigo-400 hover:shadow-2xl transition-all flex flex-col justify-between h-[300px] relative overflow-hidden"
            >
              <div className="absolute top-0 right-0 p-8 opacity-[0.02] -mr-6 -mt-6 group-hover:scale-110 transition-transform">
                <Database size={140} />
              </div>
              <div className="relative z-10">
                <div className="flex items-center justify-between mb-8">
                  ...
                  <div className="text-right">
                    <div className={`flex items-center justify-end gap-2 mb-1`}>
                      <div
                        className={`w-2 h-2 rounded-full ${item.status === 'CONNECTED' ? 'bg-emerald-500 animate-pulse' : 'bg-rose-500'}`}
                      ></div>
                      <span
                        className={`text-[10px] font-black uppercase tracking-widest ${item.status === 'CONNECTED' ? 'text-emerald-500' : 'text-rose-500'}`}
                      >
                        {item.status}
                      </span>
                    </div>
        ...
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 animate-in fade-in duration-200">
          ...
          <div className="relative bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 border border-slate-100">
            ...
            <div className="p-8 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
              <button
                onClick={() => setShowSimulator(false)}
                className="px-8 py-3 bg-white border border-slate-200 text-slate-500 text-xs font-black rounded-xl uppercase tracking-widest"
              >
                취소
              </button>
              <button
                onClick={() => {
                  alert('웹훅 패킷이 가상 큐(Queue)로 발송되었습니다.')
                  setShowSimulator(false)
                }}
                className="px-10 py-3 bg-indigo-600 text-white rounded-xl text-xs font-black uppercase tracking-widest shadow-xl shadow-indigo-500/20 active:scale-95 transition-all flex items-center gap-2"
              >
