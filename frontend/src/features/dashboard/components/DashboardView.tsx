...
const ActionCard = ({ title, count, color, icon: Icon, trend }: any) => (
  <button className="bg-white p-6 rounded-[2.5rem] border border-slate-200 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all text-left group flex flex-col justify-between h-[200px] relative overflow-hidden">
    <div
      className={`absolute -right-4 -top-4 w-24 h-24 rounded-full opacity-[0.03] group-hover:opacity-[0.08] transition-opacity ${color}`}
    ></div>
    <div>
      <div
        className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-4 transition-transform group-hover:scale-110 ${color.replace('bg-', 'bg-').replace('-500', '-50')} ${color.replace('bg-', 'text-')}`}
      >
        <Icon size={24} />
      </div>
      <h3 className="text-[11px] font-black text-slate-400 uppercase tracking-widest break-keep">
        {title}
      </h3>
    </div>
    <div className="flex items-end justify-between">
      <div className="space-y-1">
        ...
        {trend && (
          <p
            className={`text-[10px] font-bold ${trend.includes('+') ? 'text-emerald-500' : 'text-rose-500'}`}
          >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-4 py-2">
        <div>
          ...
          <p className="text-slate-500 font-medium">
            관리자님, 현재 시스템은 <span className="text-indigo-600 font-bold">정상 운영</span>{' '}
            중이며 총 4개의 채널이 동기화되고 있습니다.
          </p>
        </div>
        <div className="flex gap-2">
          <button className="px-6 py-3 bg-white border border-slate-200 rounded-2xl text-xs font-black uppercase tracking-widest text-slate-600 hover:bg-slate-50 transition-all">
            Report Center
          </button>
          <button className="px-6 py-3 bg-slate-900 text-white rounded-2xl text-xs font-black uppercase tracking-widest shadow-xl active:scale-95 transition-all">
            Manual Order
          </button>
        </div>
      </div>
          ...
          <div className="bg-slate-900 rounded-[3rem] p-8 text-white shadow-2xl relative overflow-hidden group min-h-[450px] flex flex-col">
            ...
            <div className="relative z-10 space-y-8 flex-1">
              <div className="flex items-center gap-3">
                ...
                <h4 className="text-lg font-black italic uppercase tracking-widest">
                  Operational Alerts
                </h4>
              </div>
              <div className="space-y-4">
                {[
                  {
                    label: '지연 주문',
                    msg: '3건의 주문이 48시간 이상 미출고 상태입니다.',
                    color: 'text-amber-400',
                  },
                  {
                    label: 'API 장애',
                    msg: '아마존 US 인터페이스에서 인증 오류가 감지되었습니다.',
                    color: 'text-rose-400',
                  },
                  {
                    label: '재고 부족',
                    msg: '프리미엄 자켓의 재고가 할당량 미만입니다.',
                    color: 'text-indigo-400',
                  },
                ].map((alert, i) => (
                  <div
                    key={i}
                    className="p-5 bg-white/5 border border-white/10 rounded-2xl hover:bg-white/10 transition-colors cursor-pointer group/alert"
                  >
                    <div className="flex justify-between items-center mb-1">
                      <span
                        className={`text-[10px] font-black uppercase tracking-widest ${alert.color}`}
                      >
                        {alert.label}
                      </span>
                      <ChevronRight
                        size={14}
                        className="text-white/20 group-hover/alert:text-white transition-all"
                      />
                    </div>
            ...
            <button className="relative z-10 w-full py-4 bg-white text-slate-900 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-50 transition-all shadow-xl">
              전체 경보 확인
            </button>
          </div>
        </div>
