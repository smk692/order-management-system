  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          systemInstruction: `
            You are a world-class OMS (Order Management System) AI Assistant.
            Your tone is professional, helpful, and data-driven.
            Context: The current platform is "Global OMS", an enterprise-level system.
            Key stats for today:
            - Total Sales: ₩ 14,280,000
            - Pending Orders: 45
            - Delayed Shipments: 3
            - Claims: 2
            - Popular Product: Ultra Lightweight Quilting Jacket (Stock: 156 left, selling fast)
            
            Answer questions concisely and provide actionable insights for an operations manager.
            If asked about sales or inventory, use the provided context.
          `
        }
      });

      const aiText = response.text || "죄송합니다. 응답을 생성하는 중에 문제가 발생했습니다.";
      setMessages(prev => [...prev, { role: 'assistant', content: aiText }]);
    } catch (error) {
      console.error("Gemini API Error:", error);
      setMessages(prev => [...prev, { role: 'assistant', content: "죄송합니다. AI 연결에 실패했습니다. 나중에 다시 시도해주세요." }]);
    } finally {
      setIsTyping(false);
    }
  };
